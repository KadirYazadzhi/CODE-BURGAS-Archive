...
2.  **Search**:
    ...
    *   Update `last_pos` to `found_pos + 1` (to allow overlapping if intended, though "agent" doesn't overlap with itself) or `found_pos + 5`.
3.  **Output**:
...
## Key Insights
*   For a string of $10^6$ characters, `std::cin >> string` might be slow; `std::getline` or `scanf` could be faster, depending on the format.
